...
Система конвейерной (потоковой) обработки данных --- система, состоящая из вычислительной и потребляющей частей, в которой задержка между отправкой и приёмом сообщений составляет порядка секунд--минут~\cite{streamin	g-data}.
